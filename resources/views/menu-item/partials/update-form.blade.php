    ...
    <form method="post" action="{{ route('menu-items.update', $menuItem) }}" enctype="multipart/form-data"
        ...
        <div>
            ...
            <select id="menu_category_id" name="menu_category_id"
                ...
                <option value="{{$category->id}}" @if($menuItem->menu_category_id == $category->id)  selected @endif>{{$category->title}}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="currency" name="currency"
                ...
                <option value="{{$key}}" @if($menuItem->currency === $key) selected @elseif($key==='NRS.' ) selected @endif>{{$value}}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea id="message" rows="4"
                ...
                name="description" placeholder="Write your thoughts here...">{{old('description') ?? $menuItem->description}}</textarea>
            ...
            <div class="flex items-center justify-items-center">
                <div class="flex items-center me-3">
                    <input checked id="state-1" type="radio" value="1" name="state"
                        @if(old('state') == true || $menuItem->state == true) checked @endif
                ...
                <div class="flex items-center">
                    ...
                    @if(old('state') == false || $menuItem->state == false) checked @endif
                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="state-2"
